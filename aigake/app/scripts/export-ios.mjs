...
for(const name of ['roof14-structure.json',...['match','front','right','back','left','grazing'].map(view=>'roof14-'+view+'.png')])await copySource(artStudy+'woodland/individual/cafe/evidence/'+name);
for(const name of ['roof15-structure.json',...['match','right','back'].map(view=>'roof15-'+view+'.png')])await copySource(artStudy+'woodland/individual/cafe/evidence/'+name);
await copySource(artStudy+'woodland/individual/cafe/evidence/roof16-match.png');
for(const name of ['PIXELATION-STUDY.md',...['none','weak','medium','mask'].map(mode=>'evidence/pixel1-'+mode+'.png')])await copySource(artStudy+'woodland/individual/cafe/'+name);
