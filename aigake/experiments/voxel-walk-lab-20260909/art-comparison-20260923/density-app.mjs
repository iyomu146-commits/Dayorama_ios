...
let serial=0,busy=false,scope='building',factor=2,steps=20000,playing=false,playAt=0,playSteps=0,night=false,selection=[],latest,frameStats,measuring=null,results=[],lastStats=0,lastDraws=-1,animationAt=0,ready=false;
const view=createDensityView($('density-view'),{onPick:pick,onFrame:frame});
function lock(){const blocked=busy||!!measuring;for(const id of ['density','mesh-mode','camera','night','play','restart','steps','tool','color','brush','mirror','undo','apply','clear-selection','export-work','import-work','measure','offset-x','offset-y','offset-z'])$(id).disabled=blocked;document.querySelectorAll('[data-scope],[data-steps]').forEach(b=>b.disabled=blocked);$('undo').disabled=blocked||!latest?.undo;$('apply').disabled=blocked||selection.length!==2;$('cancel-measure').hidden=!measuring;$('measure').textContent=measuring?'測定中':'25秒測る';}
...
worker.onmessage=({data:d})=>{const p=pending.get(d.id);if(!p)return;pending.delete(d.id);busy=pending.size>0;$('busy').hidden=true;
 ...
 if(d.type==='geometry'){
  const applyStart=performance.now();view.load(d);const applyMs=performance.now()-applyStart;if(d.complete)d.complete.items=[];d.active.items=[];latest=d;factor=d.factor;steps=d.steps;$('density').value=String(factor);
  $('model-count').textContent=scope==='district'?`5棟 · 樹5本 · 住民${night?'0':'12'}人`:`建物・テラス ${fmt(d.completed.count)}粒`;
  $('scene-status').textContent=scope==='district'?`手前右の1棟を建築 · ${fmt(d.completed.count*4+d.visible.count)}粒`:`主屋の幅${factor===2?32:64}粒`;
  ...
  if(d.edited)$('selection-status').textContent=`${fmt(d.edited)}粒を変更`;
...
function clearSelection(){selection=[];view.select();$('apply').disabled=true;$('selection-status').textContent='模型の2か所を押して範囲を選択';}
...
async function setScope(next){scope=next;$('editor').hidden=scope==='district';$('tool').value='view';view.edit(false);document.querySelectorAll('[data-scope]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.scope===scope)));await load();}
...
async function pick(q,n){
 if(busy||measuring||scope!=='building'||steps!==20000||$('tool').value==='view')return;
...
$('tool').onchange=safe(async()=>{stop();clearSelection();if(steps!==20000)await progress(20000);view.edit($('tool').value!=='view');if($('tool').value==='extrude')$('selection-status').textContent='追加する面を押してください';$('offsets').hidden=$('tool').value!=='copy';});$('clear-selection').onclick=clearSelection;
...
$('export-work').onclick=safe(async()=>{const d=await request('export');save(`dayorama-density-${factor}.json`,d.text);});$('import-work').onclick=()=>$('work-file').click();$('work-file').onchange=safe(async e=>{const f=e.target.files[0];if(!f)return;if(f.size>24000000)throw Error('作品ファイルが大きすぎます');stop();clearSelection();steps=20000;await request('import',{text:await f.text()},true);e.target.value='';});
...
lock();safe(async()=>{await load();ready=true;lock();})();
