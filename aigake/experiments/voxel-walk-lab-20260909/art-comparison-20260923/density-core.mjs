...
export function editBox(map,{a,b,tool,color='roof',mirror=false,offset=[0,0,0]}){
 ...
 const put=(q,c)=>{if(q.some(v=>Math.abs(v)>160)||q[1]<0)return;const key=coordKey(q);if(!changes.has(key))changes.set(key,{key,old:map.get(key)});if(c)next.set(key,{...c,x:q[0],y:q[1],z:q[2],under:undefined,phase:4,group:'cafe',emission:c.color.startsWith('glass')});else next.delete(key);};
...
export function encodeDensity(cells,factor){return JSON.stringify({format:'dayorama-density-study',version:1,factor,cell:CELL*factor,cells:[...cells.values()].map(({x,y,z,color,phase,group,under})=>({x,y,z,color,phase,group,under}))});}
