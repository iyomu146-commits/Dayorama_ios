...
export class DensityWorkspaces{
 ...
 preview(scope,factor,command){if(scope==='district')throw Error('編集画面で操作してください');return editBox(this.get(scope,factor).map,{...command,bounds:scope==='blank'?workbenchBounds(factor):null,support:scope==='blank'?(command.support==='below'?'below':'connected'):'none'});}
 edit(scope,factor,command){const state=this.get(scope,factor),result=this.preview(scope,factor,command);state.map=result.map;state.lastBlocked=result.blocked.length;this.remember(state,result.patch);return result.patch.length;}
