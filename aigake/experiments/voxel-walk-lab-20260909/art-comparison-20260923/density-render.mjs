...
import {WORKBENCH_SIZE,workbenchBounds} from './density-core.mjs';
...
export function createDensityView(host,{onPick,onFrame}){
 ...
 const hover=new THREE.Box3Helper(new THREE.Box3(new THREE.Vector3(),new THREE.Vector3()),0x678e75);hover.visible=false;hover.material.depthTest=false;hover.renderOrder=9;scene.add(hover);let editTool='view',brush=1,lastHover=0;
 ...
 function load(data){const oldScope=scope,oldFactor=factor;scope=data.scope;factor=data.factor;steps=data.steps;hover.visible=false;controls.enableRotate=scope==='blank'||!editing;controls.enablePan=scope==='blank';controls.maxZoom=scope==='blank'?8:4;if(scope==='blank'&&(!grid||oldFactor!==factor))updateGrid();
  ...
  const size=scope==='district'?31:9;Object.assign(sun.shadow.camera,{left:-size,right:size,top:size,bottom:-size,near:.5,far:90});sun.shadow.camera.updateProjectionMatrix();sun.position.set(scope==='district'?-15:-5,scope==='district'?30:11,scope==='district'?22:9);visibility();if(oldScope!==scope){cameraAngle();resize();}dirty=shadowDirty=true;
 }
 const ray=new THREE.Raycaster(),pointer=new THREE.Vector2();
 function hitCell(e){const r=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1);ray.setFromCamera(pointer,camera);let hit=ray.intersectObjects([...active.values()],false)[0];
 ...
 host.addEventListener('pointermove',e=>{if(!editing||scope!=='blank'||e.timeStamp-lastHover<30)return;lastHover=e.timeStamp;const hit=hitCell(e);hover.visible=false;if(hit&&(['place','extrude'].includes(editTool)||!hit.floor)){
   const add=['place','extrude'].includes(editTool),q=hit.q.map((v,i)=>v+(add?hit.n[i]:0)),radius=add?(brush-1)/2:0,bounds=workbenchBounds(factor),lo=q.map((v,i)=>Math.max(bounds.min[i],v-(hit.n[i]?0:radius))),hi=q.map((v,i)=>Math.min(bounds.max[i],v+(hit.n[i]?0:radius)));
