...
check('Glazed lean-to roof exposes its glass instead of a solid covering',()=>{for(let x=-31;x<-17;x++)for(let z=2;z<17;z++)assert.equal(get(x,17-Math.floor((z-1)/3),z)?.part,'greenhouse-glass');});
check('Front, side, door and greenhouse panes have no vertical bars',()=>{
 ...
 for(let x=-31;x<-17;x++)for(let y=2;y<11;y++)assert(get(x,y,17)?.part==='greenhouse-glass');
});
