...
r=run('forge/stage4_review/diagnose_render.py',['--reference','evidence/reference-mask.png','--render',f'evidence/bakery-{stage}-match.png','--map-stripped-render',f'evidence/bakery-{stage}-match-mask.png','--spec','sculpt-spec.json','--pass-id',pass_id,'--in-place','--json'],f'evidence/{stage}-diagnostics.json',colored)
json.loads(r.stdout)
if colored:
...
run('forge/stage4_review/diagnose_render_multi_angle.py',['--reference',f'evidence/bakery-{stage}-match-mask.png','--orbit',f'evidence/bakery-{stage}-front-mask.png','--orbit',f'evidence/bakery-{stage}-right-mask.png','--json'],f'evidence/{stage}-multi-angle.json')
run('forge/stage4_review/turntable_gate.py',['--capture',f'0=evidence/bakery-{stage}-front-mask.png','--capture',f'90=evidence/bakery-{stage}-right-mask.png','--capture',f'180=evidence/bakery-{stage}-back-mask.png','--capture',f'270=evidence/bakery-{stage}-left-mask.png','--allow-holes','--json'],f'evidence/{stage}-turntable.json')
...
sheet=Image.new('RGB',(1155,710),'#eeece5');draw=ImageDraw.Draw(sheet)
files=[Path('../../references/crops/bakery.png')]+[Path(f'evidence/bakery-{stage}-{v}.png') for v in ['match','front','right','back','left']]
for i,p in enumerate(files):
 im=Image.open(p).convert('RGB');im.thumbnail((385,333));x=i%3*385;y=i//3*355;sheet.paste(im,(x,y+22));draw.text((x+8,y+4),p.stem,fill='#393e38')
