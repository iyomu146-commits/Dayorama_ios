...
export function createCafeCells({stage='structure',color=true}={}){
 ...
 const map=new Map();
 function put(part,x,y,z,c,phase=2){map.set(`${x},${y},${z}`,{x,y,z,part,color:color?c:'clay',phase});}
 ...
 cut(-14,5,12,17,14,3);cut(15,5,-9,3,14,16);cut(5,2,12,10,18,3);
 // Window centre panes are uninterrupted. No central structural mullions.
 box('front-window',-14,5,12,17,14,3,'ivory');
 box('front-window',-13,6,14,15,12,1,'wood');
 box('front-window',-12,7,13,13,10,1,(x,y)=>y<10+Math.floor((x+12)/4)?'glassMid':x<-8&&y>13?'glassPale':'glass');cut(-12,7,14,13,10,1);
 box('front-window',-15,4,13,19,1,3,'ivory');
 box('side-window',15,5,-9,3,14,16,'ivory');
 box('side-window',17,6,-8,1,12,14,'wood');
 box('side-window',16,7,-7,1,10,12,(_x,y,z)=>y<10+Math.floor((z+7)/4)?'glassMid':'glass');cut(17,7,-7,1,10,12);
 box('side-window',16,4,-10,3,1,18,'ivory');
 ...
 for(let x=-18;x<18;x++){const c=Math.floor((x+18)/3)%2?'ivory':'cloth';box('awning',x,21,14,1,1,3,c,3);box('awning',x,20,17,1,1,2,c,3);put('awning',x,19,18,c,3);}
 ...
 // Five wide lanes, six courses. Repair colors are explicit, not hash noise.
 ...
 for(const sign of [-1,1])for(let lane=0;lane<5;lane++)for(let row=0;row<6;row++){
  const x=-20+lane*8,z=sign>0?15-row*3:-18+row*3,y=23+row*2,c=tones[lane][row];
  box('tiles',x,y,z,7,1,3,c,3);box('tiles',x+7,y,z,1,1,3,'slateDark',3);
  box('tiles',x,y+1,z+(sign>0?1:0),7,1,2,c,3);
  // Two- or three-cell patches stay within the tile; never per-cell random noise.
  const patchStarts=[[0,3,1,4,2,0],[2,0,4,1,0,3],[4,1,0,3,2,4],[1,4,2,0,3,1],[3,0,2,4,1,0]],px=patchStarts[lane][row],pc=c.startsWith('slateSage')?'slateSagePatina':c==='slateMuted'?'slate':'slatePatina';
  box('tiles',x+px,y+1,z+(sign>0?1:0),2+(row%2),1,2,pc,3);
 }
 box('ridge',-20,34,-1,40,2,2,(x)=>Math.floor((x+20)/4)%3===1?'slateMuted':'slateEdge',3);
 // Back of chimney is inferred; shaft starts within the roof support, cap has a real opening.
 box('chimney',-13,30,-5,5,7,5,(x,y,z)=>brick(x,y,z),3);box('chimney',-14,37,-6,7,1,7,'ivory',3);cut(-12,35,-4,3,3,3);
 ...
 box('table',24,8,5,8,1,8,(x)=>x%3===0?'woodLight':'wood',4);
