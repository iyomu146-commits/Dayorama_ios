...
check('Front and side windows retain uninterrupted glass, no centre posts',()=>{for(let x=-11;x<=-1;x++)for(let y=8;y<=15;y++){assert(cells.get(`${x},${y},13`).color.startsWith('glass'));assert(!cells.has(`${x},${y},14`));}for(let z=-5;z<=3;z++)for(let y=8;y<=15;y++){assert(cells.get(`16,${y},${z}`).color.startsWith('glass'));assert(!cells.has(`17,${y},${z}`));}});
...
check('Chimney has a continuous open flue through its cap',()=>{for(let x=-12;x<=-10;x++)for(let z=-10;z<=-8;z++)for(let y=33;y<=40;y++)assert(!cells.has(`${x},${y},${z}`));});
