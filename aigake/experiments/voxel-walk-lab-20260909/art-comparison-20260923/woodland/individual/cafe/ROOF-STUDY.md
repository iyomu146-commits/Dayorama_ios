...
## 現行：継ぎ目の明度調整 / 2026-09-24・roof16

「つなぎ列の色がちょっと暗すぎる」という指摘を受け、継ぎ目専用色を深い青緑 `#233f48` から `#34545b` へ明るくした。2列＋2列＋継ぎ目1列の反復・形状・材質・照明は維持。旧配色での「屋根のみ」比較にも専用色 `#386368` を定義する。実ブラウザーで黒い溝の印象が弱まり、控えめな区切りとして残ることを確認。数値は14,335セル・11,804三角形のまま。

![明度調整後](./evidence/roof16-match.png)

## 以前：2列＋2列＋暗い継ぎ目1列 / 2026-09-24・roof15

ユーザーへ「15cmボクセルで瓦2列＋少し色の違う瓦2列＋暗い継ぎ目1列を横に繰り返す」と確認し、「その繰り返しでよい」と回答を受けた。屋根幅40セルに5セル周期を8回置く。瓦2列は30cm、継ぎ目は15cm。奥行きの6段・屋根の座標・15cm単位はそのまま、色を変える。暗い列も同じ高さのセルで埋め、溝や穴にしない。棟は別部材として連続させる。

14,335セル・11,804三角形・22部材。色の境界が増え、roof14より三角形は1,534増えた。各段の2＋2＋1配色、全列の接続、接地、煙突と入口、屋根以外のセル、整数セルの外周と描画面の一致を検査。実ブラウザーの参照視点・側面・裏面を保存し、エラーなし。今回は喫茶店のみ。住宅とパン屋には適用していない。正式な品質ゲートの保留状態は維持する。

![継ぎ目を明るくする前](./evidence/roof15-match.png)

## 以前：パン屋に合わせた粒度 / 2026-09-24・roof14
...
![5列にまとめた版](./evidence/roof14-match.png)
