...
check('All retained parts exist; user-requested lamp removal changes nothing else',()=>{const spec=JSON.parse(fs.readFileSync('sculpt-spec.json'));const parts=new Set([...cells.values()].map(c=>c.part));assert.deepEqual(parts,new Set(spec.componentTree.map(c=>c.id).filter(id=>stage!=='no-lamp'||!id.startsWith('lamp-'))));if(stage==='no-lamp'){const previous=createBooksCells({stage:'optimization'});for(const [key,c] of previous)if(!c.part.startsWith('lamp-'))assert.deepEqual(cells.get(key),c);assert([...cells.values()].every(c=>!c.part.startsWith('lamp-')));}});
