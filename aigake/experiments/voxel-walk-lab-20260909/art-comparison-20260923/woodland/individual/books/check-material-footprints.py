...
render_path=Path(f'evidence/books-{stage}-match.png'); render=Image.open(render_path).convert('RGBA')
...
for component in spec['componentTree']:
 name=component['id'];mask_path=Path(f'evidence/books-{stage}-footprint-{name}.png')
...
for name in ['building','roof','display-glass','books-lower','books-upper','door','bench','herb-left','herb-right','chimney-rim']:
