...
render_path=Path(f'evidence/home-{stage}-match.png'); render=Image.open(render_path).convert('RGBA')
...
for component in spec['componentTree']:
 name=component['id'];mask_path=Path(f'evidence/home-{stage}-footprint-{name}.png')
...
for name in ['roof','chimney','lower-wall','upper-wall','front-low','front-upper-left','front-upper-right','door','porch','rail-left','rail-right','chimney-cap']:
